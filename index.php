<!doctype html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Library CSS -->
    <link rel="stylesheet" href="assets/bootstrap/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="style.css">
    <link href="assets/animate/animate.min.css" rel="stylesheet">
    <link href="assets/magnific-popup/magnific-popup.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Roboto:400,700" rel="stylesheet">

    <!-- Main css -->
    <title>Jbay - Surf & SUP Bali</title>
  </head>
  <body>

    <!--==========================
      Header
    ============================-->
    <header id="header">
      <div class="container">

        <div id="logo" class="pull-left">
          <!-- <h1><a href="index.html" class="scrollto">Jbay</a></h1> -->
          <!-- Uncomment below if you prefer to use an image logo -->
          <a href="index.html"><img src="assets/images/logo-black.png" height="40" alt="" title=""></a>
        </div>

        <nav id="nav-menu-container">
          <ul class="nav-menu">
            <li class="menu-active"><a href="index.html">Home</a></li>
            <li class="menu-has-children"><a href="#">Course</a>
              <ul>
                <li><a href="surfing.html">Surfing</a></li>
                <li><a href="sup.html">Stand Up Paddle</a></li>
                <li><a href="bodyboarding.html">Bodyboarding</a></li>
              </ul>
            </li>
            <li class="menu-has-children"><a href="services.html">Services</a>
              <ul>
                <li><a href="boat_taxy.html">Boat Taxy</a></li>
                <li><a href="surf_guiding.html">Surf Guiding</a></li>
                <li><a href="sup_guiding.html">SUP Guiding</a></li>
                <li><a href="sunset_trip.html">Sunset Trip</a></li>
                <li><a href="fishing.html">Fishing</a></li>
              </ul>
            </li>
            <li><a href="rental.html">Rental</a></li>
            <!-- <li><a href="about_us.html">About Us</a></li> -->
            <li><a href="contact.html">Contact</a></li>
          </ul>
        </nav><!-- #nav-menu-container -->
      </div>
    </header><!-- #header -->

    <!-- intro section -->
    <section id="intro">
      <div class="container">
        <div class="intro-text text-center">
          <h2 class="fadeInUp animated">Welcome to Jbay</h2>
          <p class="fadeInUp animated" data-caption-delay="200">Create just what you need for your Perfect Website. Choose from a wide range of Elements & simply put them on your own Canvas.</p>
          <a href="#about" class="btn-get-started scrollto">Get Started</a>
        </div>
      </div>
    </section>

    <!-- video section -->
    <!-- <div class="container my-5">
     <div class="videoWrapper">
       Copy & Pasted from YouTube
      <iframe width="560" height="100" src="https://www.youtube.com/embed/4LdnySqs1Ks?rel=0&hd=1" frameborder="0" allowfullscreen></iframe>
      </div>
    </div> -->
    <!-- video ends here -->

    <!-- about_us section -->
    <section id="about-us" class="py-3 bg-darkgray">
      <div class="container my-5">
        <div class="row">

          <div class="col-md-6">
          <div class="about-content title mb-3">
            <h2>
              About Us <br>
            </h2>
            <p class="mb-3">
              Our mission has always been to provide each and every student with the ultimate experience on every step on their search, fromthe pick up, the welcome at the reception, the best-in-class facilities, equipment and our educational programs. We aim to ne a cool and vibrant place of learning. where everyone cam discover their search and share their experience with other who also share the passion for adventurer and respect for our oceans. Thus, our strong relationship with Rip Curl Family.
            </p>
            <br>
            <a href="#"><button class="btn btn-black">Learn More</button></a>
          </div>
        </div>

          <div class="col-md-6">
            <img src="assets/images/jbay-about.jpg" class="img-fluid rounded" alt="">
          </div>
        </div>

      </div>
    </section>

    <!-- course section -->
    <section id="courses" class="pt-5">
      <div class="container">
        <div class="text-center mb-3">
          <h2 class="mb-2">Course</h2>
        <p class="mb-2">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis, dicta!</p>
        </div>
        <div class="services-pic">
          <div class="row">

            <div class="col-md-4">
              <figure>
                <img src="https://source.unsplash.com/nX2tkqsf_eI/1600x900" alt="">
                <figcaption>

                </figcaption>
                <a class="link" href="surfing.html">
                  <div class="caption">
                    <h3>SURFING</h3>
                  </div>
                </a>
              </figure>
            </div>

            <div class="col-md-4">
              <figure>
                <img src="https://source.unsplash.com/KgCbvOWYuU0/1600x900" alt="">
                <figcaption>

                </figcaption>
                <a class="link" href="bodyboard.html">
                  <div class="caption">
                    <h3>SUP</h3>
                  </div>
                </a>
              </figure>
            </div>

            <div class="col-md-4">
              <figure>
                <img src="https://source.unsplash.com/CIMet4T90Ng/1600x900" alt="">
                <figcaption>

                </figcaption>
                <a class="link" href="bodyboard.html">
                  <div class="caption">
                    <h3>BODYBOARDING</h3>
                  </div>
                </a>
              </figure>
            </div>

          </div>
        </div>
      </div>
    </section>

    <!-- services list -->
    <div class="course-section mt-5 py-5 bg-darkgray">
      <div class="container">
        <div class="course text-center mb-3 p-3">
          <h2 class="heading-title">SERVICES</h2>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Tempore, obcaecati.</p>
        </div>
        <div class="row">
          <div class="col-md-4 bg-tall no-gutters fadeInUp animated wow">
            <div class="box1 bg-surfing">

            </div>
            <a class="link" href="pulau.html">
                      <div class="caption">
              <h3>PULAU OUTFITTERS</h3>
            </div>
            </a>
          </div>
          <div class="col-md-8 no-gutters">
            <div class="row">
              <div class="col-md-6 bg-small fadeInUp animated wow">
                <div class="box1 bg-sup">

                </div>
                      <a class="link" href="boat_taxy.html">
                      <div class="caption">
              <h3>BOAT TAXY</h3>
            </div>
            </a>
            </div>
            <div class="col-md-6 bg-small no-gutters fadeInUp animated wow">
              <div class="box1 bg-bodyboard">

              </div>
                    <a class="link" href="surf_guiding.html">
                      <div class="caption">
              <h3>SURF GUIDING</h3>
            </div>
            </a>
            </div>
                    <div class="col-md-12 bg-small fadeInUp animated wow">
              <div class="box1 bg-daypass">

                      </div>
                            <a class="link" href="sup_guiding.html">
                      <div class="caption">
              <h3>SUP GUIDING</h3>
            </div>
            </a>
            </div>
            </div>    <!--   row -->
        </div>
          <div class="col-md-6 bg-small fadeInUp animated wow">
            <div class="box1 bg-seakayak">

            </div>
                            <a class="link" href="sunset_trip.html">
                      <div class="caption">
              <h3>SUNSET TRIP</h3>
            </div>
            </a>
          </div>
          <div class="col-md-6 bg-small fadeInUp animated wow">
            <div class="box1 bg-pulau">

            </div>
                                  <a class="link" href="fishing.html">
                      <div class="caption">
              <h3>FISHING</h3>
            </div>
            </a>
          </div>
      </div> <!--   row -->
      </div> <!--   container -->
    </div>
    <!-- services list -->

    <!-- feature list -->
    <!-- <div class="feature my-5 mx-3">

      <div class="container">

        <div class="heading-block center text-center w-700">
          <h2 class="heading-title">Why Choose US</h2>
          <p>We provide a wide array of Usable &amp; Flexible Features</p>
        </div>

        <div class="row">

          <div class="col-lg-4">
            <img src="assets\images\surfer-black.png" alt="iPhone" class="center-block fadeInLeft animated wow">
          </div>

          <div class="col-lg-4 col-md-6 mt-5">

            <div class="col_full">

              <div class="feature-box fbox-plain fbox-small fbox-dark mb-4 fadeInUp animated wow">
                <div class="fbox-icon">
                  <a href="#"><i class="fa fa-car fa-lg"></i></a>
                  <h3 class="ml-2">Free Transport</h3>
                </div>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Est quae totam quos, expedita earum laudantium minima ut commodi maxime rem.</p>
              </div>

            </div>

            <div class="col_full">

              <div class="feature-box fbox-plain fbox-small fbox-dark mb-4 fadeInUp animated wow">
                <div class="fbox-icon">
                  <a href="#"><i class="fa fa-angellist fa-lg"></i></a>
                  <h3 class="ml-2">Certified Instructor</h3>
                </div>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Fugit facere vel porro, corrupti dolor reprehenderit itaque ex pariatur ipsum dicta!</p>
              </div>

            </div>

            <div class="col_full">

              <div class="feature-box fbox-plain fbox-small fbox-dark mb-4 fadeInUp animated wow">
                <div class="fbox-icon">
                  <a href="#"><i class="fa fa-ambulance fa-lg"></i></a>
                  <h3 class="ml-2">Insurance</h3>
                </div>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Saepe molestias eius, similique hic, dolorum mollitia ullam. Quam, aliquam dolorum ad.</p>
              </div>

            </div>

          </div>

          <div class="col-lg-4 col-md-6 mt-5">

            <div class="col_full">

              <div class="feature-box fbox-plain fbox-small fbox-dark mb-4 fadeInUp animated wow">
                <div class="fbox-icon">
                  <a href="#"><i class="fa fa-cog fa-lg"></i></a>
                  <h3 class="ml-2">Top Grade Equipment</h3>
                </div>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cum, error accusamus eligendi repudiandae, quo a ipsa. Fuga, aliquam facilis ut.</p>
              </div>

            </div>

            <div class="col_full">

              <div class="feature-box fbox-plain fbox-small fbox-dark mb-4 fadeInUp animated wow">
                <div class="fbox-icon">
                  <a href="#"><i class="fa fa-shower fa-lg"></i></a>
                  <h3 class="ml-2">Guest Facilities</h3>
                </div>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Distinctio praesentium nemo voluptatum exercitationem deleniti mollitia officiis laudantium eveniet est maiores?</p>
              </div>

            </div>

            <div class="col_full">

              <div class="feature-box fbox-plain fbox-small fbox-dark fadeInUp animated wow">
                <div class="fbox-icon">
                  <a href="#"><i class="fa fa-sun-o"></i></a>
                  <h3 class="ml-2">Suncreen & SYNC</h3>
                </div>
                <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Earum quo quasi inventore, explicabo quam impedit praesentium esse rem architecto iure?</p>
              </div>

            </div>

          </div>

        </div>

      </div>

    </div> -->
    <!-- feature list end -->

    <!-- call to action section -->
    <section id="cta" class="maps-cta-section text-white">
      <header class="cta jb-hero jb-hero-solutions maps-full-bleed" >
        <div class="jb-hero-content">
          <h1 class="hero-title">add call to action desc..here</h1>
          <div class="hero-subtitle">
            <div class="hero-additional-content">
              <button class="btn btn-lg btn-primary">Lets'go</button>
            </div>
          </div>
        </div>
      </header>
    </section>

    <!-- partners section -->
    <div class="partners nobottommargin clearfix py-5">

      <div class="container clearfix">
        <div class="heading-block noborder w-700 text-center">
          <h2 class="mb-4">Our Partners</h2>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Alias, provident.</p>
        </div>
        <ul class="clients-grid grid-4 nobottommargin clearfix pl-0">
          <li style="padding: 20px;"><a href="https://www.bali.intercontinental.com/" target="_blank"><img src="assets/images/logo_intercon2.png" alt="Clients"></a></li>
          <li style="padding: 20px;"><a href="https://www.pulauoutfitters.com/" target="_blank"><img src="assets/images/logo_pulau2.png" alt="Clients"></a></li>
          <li style="padding: 20px;"><a href="https://www.ripcurl.asia/id/" target="_blank"><img src="assets/images/logo_rc.png" alt="Clients"></a></li>
          <li style="padding: 20px;"><a href="http://www.shiftdivision.com/" target="_blank"><img src="assets/images/logo_shiftdivision.png" alt="Clients"></a></li>
        </ul>
      </div>
    </div>
    <!-- partners section ends-->


    <!--==========================
      Footer
    ============================-->
    <footer>
      <div id="footer" class="mt-2 bg-dark p-3">
      <div class="container p-1">
        <div class="row">
          <div class="col-6 col-md-3 col-sm-6">
            <h5>HELP</h5>
            <a href="FAQ.html">FAQs</a> <br>
            <a href="studybooks.html">Study Books</a> <br>
            <a href="tide_cart.html">Tide Cart</a> <br>
            <a href="gallery.html">Gallery</a> <br>
          </div>
          <div class="col-6 col-md-3 col-sm-6">
            <h5>QUICK LINKS</h5>
            <a href="about_us.html">About Us</a> <br>
            <a href="partners.html">Partners</a> <br>
            <a href="https://www.ripcurl.asia/en/" target="_blank">Rip Curl Store</a> <br>
            <a href="privacy.html">Privacy Policy</a> <br>
            <a href="term.html">Term of Use</a> <br>
          </div>
          <div class="col-md-3 col-sm-6">
            <div class="footer-social-icons">
              <h5 class="_14">Follow us on</h5>
              <ul class="social-icons mb-3">
                <li><a href="https://www.facebook.com/jbaysurfandsupbali/" target="_blank"> <i class="fa fa-facebook"></i></a></li>
                <li><a href="https://www.instagram.com/jbaysurfandsup_bali/" target="_blank" class="social-icon"> <i class="fa fa-instagram"></i></a></li>
                <li><a href="https://www.tripadvisor.com/" target="_blank" class="social-icon"> <i class="fa fa-tripadvisor"></i></a></li>
                <!-- <li><a href="" class="social-icon"> <i class="fa fa-youtube"></i></a></li>
                <li><a href="" class="social-icon"> <i class="fa fa-linkedin"></i></a></li>
                <li><a href="" class="social-icon"> <i class="fa fa-google-plus"></i></a></li> -->
              </ul>
              <ul class="social-icons mb-3">
                <!-- <li><a href="https://www.facebook.com/jbaysurfandsupbali/" target="_blank"> <i class="fa fa-facebook"></i></a></li>
                <li><a href="https://www.instagram.com/jbaysurfandsup_bali/" target="_blank" class="social-icon"> <i class="fa fa-instagram"></i></a></li>
                <li><a href="https://www.tripadvisor.com/" target="_blank" class="social-icon"> <i class="fa fa-tripadvisor"></i></a></li> -->
                <li><a href="" class="social-icon"> <i class="fa fa-youtube"></i></a></li>
                <li><a href="" class="social-icon"> <i class="fa fa-linkedin"></i></a></li>
                <li><a href="" class="social-icon"> <i class="fa fa-google-plus"></i></a></li>
              </ul>
            </div>

          </div>
          <div class="col-md-3 col-sm-6">
            <div class="footer-address">
              <h5>Contact</h5>
              <p>Intercontinental Bali resort <br>
                Jl. Ulutuwatu No. 45 Jimbaran Bali Indonesia 80361
              </p>
              <a href="tel:62-361-701888"><i class="fa fa-phone-square" target="_blank"></i> +62 361 701888 ext. 87605</a><br>
              <a href="tel:62-812-4639-1487"><i class="fa fa-phone-square" target="_blank"></i> +62 812 4639 1487</a> <br>
              <a href="https://api.whatsapp.com/send?phone=6281246391487" target="_blank"><i class="fa fa-whatsapp"></i> +62 812 4639 1487</a> <br>
              <a href="mailto:info@jbaysurfandsupbali.com" class="fa fa-envelope-o"> info@jbaysurfandsupbali.com</a>
            </div>
          </div>
        </div>
      </div>
      </div>
      <!-- end of footer   -->
      <div class="copyright text-center p-2">
        <p class="small">
          © Copyright <strong>J-Bay</strong>. All Rights Reserved</p>
      </div>
    </footer>




    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <link href="assets/ionicons/css/ionicons.min.css" rel="stylesheet">
    <script src="assets/jquery/jquery.min.js"></script>
    <script src="assets/jquery/jquery-migrate.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js"></script>
    <script src="assets/easing/easing.min.js"></script>
    <script src="assets/wow/wow.min.js"></script>
    <script src="assets/superfish/hoverIntent.js"></script>
    <script src="assets/superfish/superfish.min.js"></script>
    <script src="assets/magnific-popup/magnific-popup.js"></script>
    <script src="script.js" charset="utf-8"></script>
  </body>
</html>
